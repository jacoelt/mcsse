...
export type SearchParams = {
  ...
  version?: string[]; // Array of versions to filter by
  edition?: Edition; // Edition filter
  ...
  status?: ("online" | "offline" | "unknown")[]; // Server status
  total_votes_min?: number; // Minimum total votes
  total_votes_max?: number; // Maximum total votes
  country?: string[]; // Array of countries to filter by
